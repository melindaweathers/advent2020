class ConwayCubes
  attr_accessor :space
  CHANGES = [0,1,-1].repeated_permutation(3).to_a - [[0,0,0]]
  def initialize(filename)
    @space = {}
    IO.readlines(filename, chomp: true).each_with_index do |line, y|
      line.chars.each_with_index {|char, x| @space["#{x} #{y} 0"] = char}
    end
  end
...
puts ConwayCubes.new('sample.txt').boot
puts ConwayCubes.new('input.txt').boot
